...
//        binding.etProductDesc.setText("Wedding loan for members")
    ...
    private fun saveLoanProduct() {
        if(validated()){
            ...
            jsonContribution["interesttype"] = binding.etInterestType.text.toString().toLowerCase()
            jsonContribution["interestvalue"] = binding.etInterest.text.toString()
            jsonContribution["max_principal"] = binding.etMaxAmount.text.toString()
            jsonContribution["min_principal"] = binding.etMinAmount.text.toString()
            jsonContribution["paymentperiod"] = binding.etPaymentPeriod.text.toString()
            jsonContribution["paymentperiodtype"] = binding.etPaymentPeriodType.text.toString().toLowerCase()
            jsonContribution["productname"] = binding.etProductName.text.toString()
            jsonContribution["description"] = binding.etProductDesc.text.toString()
