...
class LoanPaymentRequestAdapter(val context: Fragment, val paymentList:ArrayList<Payment>):RecyclerView.Adapter<LoanPaymentRequestAdapter.RecentActivityViewHolder> (){
    ...
    fun showActionDialogue(action:String,position: Int){
        var title = "Approve"
        var msg = "Do you want to approve loan payment"
        if(action.contentEquals("decline")){
            title = "Reject"
            msg = "Do you want to reject loan payment"
        }
        ...
        builder.setPositiveButton("Yes"){dialogInterface, which ->
            groupModel.myGroupApiResponseLD.postValue(MyApiResponse(700,"acceptDeclineGrpLoanPayment","",""))

            val jsonDeclineDetails = HashMap<String,Any>()
            jsonDeclineDetails["loanpaymentrecord"] =  paymentList[position].paymentid
            if(action.contentEquals("approve")){
                jsonDeclineDetails["action"] = "approve"
                groupModel.approveDeclineGrpLoanPaymentRequest(jsonDeclineDetails)
            }else{
